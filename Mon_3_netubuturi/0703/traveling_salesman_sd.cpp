...
typedef struct {double x; double y;} SITE; // 構造体 SITE を定義する
...
PICKUP_J:
    ...
    if (i==j) goto PICKUP_J;    // i と j が同じなら選び直し

    exchange(i,j);                 // ２つのサイトを交換
    dtemp=calc_dist( );            // 新しい経路での距離を計算

    if (dtemp<distance) { // 交換すると距離が減少したので新経路を採用（Steepest Descent）
      distance=dtemp;
    } else {              // 交換すると距離が増加したので元に戻す
      exchange(i,j);
    }

     printf(      "%10d %12.6f\n",step,distance);
    fprintf(fdist,"%10d %12.6f\n",step,distance);
    if (step%OUTSTEP==0) {  // OUTSTEP ごとに経路データを出力
      output(step);
    }
  }
