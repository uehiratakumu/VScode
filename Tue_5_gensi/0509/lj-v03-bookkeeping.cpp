...
double DEL_T  = 0.001;
double CELL_X;             // Cell size is determined by Density
double CELL_Y;
double CELL_Z;
...
int neigh[NUM_ATOM], nlist[MAX_NEIGH][NUM_ATOM];
...
int main( )
{
  ...
  for (step=0; step<=TOTAL_STEP; step++) {
    if (step%BOOK_STEP==0) list_update( );
    force( );
    move( );
...
void initplot( )
{
  ...
  fprintf(fgnuplot,"set key out font \',20\'\n");
...
void initial( )
{
  ...
  CELL_X=pow(NUM_ATOM/DENSITY, 1.0/3.0);
...
void list_update( )
{
  ...
  for (j=i+1; j<NUM_ATOM; j++) {
    ...
    if (r2<RTBL2) {
      ...
      in++;
      if (in==MAX_NEIGH) {
        printf("Warning: Number of Neighbors exceeds MAX_NEIGH\n");
        in--;
      }
    }
  }
...
void force( )
{
  ...
  for (i=0; i<NUM_ATOM; i++) {
  for (in=0; in<neigh[i]; in++) {   // Refer to Neighbor List
    j=nlist[in][i];
